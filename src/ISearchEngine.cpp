...
std::vector<song>
ISearchEngine::query (const std::string &lyric_to_find)
{
  auto *logger = get_logger ();
  CLOG (INFO, logger->id ().c_str ()) << "Searching for " << get_first_ten_words (lyric_to_find);
...
void
print_results (const std::vector<song> &results)
{
