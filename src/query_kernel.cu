...
__device__ bool
is_lyric_in_song (const char *lyric, const char *song)
{
  int i = 0;
  int j = 0;
...
__global__ void
query_kernel (const char *lyric_to_search, const char *d_lyrics,
...
void
launch_query_kernel (const char *lyric_to_search, int lyric_len,
					 ...
					 float *kernel_time_ms, float *data_transfer_time_ms)
{
  ...
  cudaEventElapsedTime (&dt_ms, data_transfer_start,
						data_transfer_stop);
