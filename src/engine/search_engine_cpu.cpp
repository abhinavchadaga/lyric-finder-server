...
search_engine_cpu::search_engine_cpu(const char *path_to_db, size_t nthreads)
    : ISearchEngine(), m_nthreads(nthreads) {
  std::string identity{"search_engine_cpu_" + std::to_string(m_nthreads) +
                       "_threads"};
  m_logger = el::Loggers::getLogger(identity);
  CLOG(INFO, m_logger->id().c_str())
      << "creating search_engine_cpu (using " << m_nthreads << " threads)";
...
std::vector<song> search_engine_cpu::query_internal(
...
el::Logger *search_engine_cpu::get_logger() { return m_logger; }

std::vector<song> search_engine_cpu::query_seq(
    const std::string &lyric_to_find) {
  std::vector<song> result;
  const auto start = std::chrono::high_resolution_clock::now();

  std::copy_if(m_db.begin(), m_db.end(), std::back_inserter(result),
               [&lyric_to_find](const song &s) {
  ...
  CLOG(INFO, m_logger->id().c_str())
      << "Search completed in " << duration.count() << " milliseconds";

  return result;
}

std::vector<song> search_engine_cpu::query_par(
    const std::string &lyric_to_find) {
  ...
  for (size_t tid = 0; tid < m_nthreads; ++tid) {
    threads[tid] =
        std::thread(&search_engine_cpu::search_worker, this,
                    std::ref(lyric_to_find), tid, std::ref(t_results[tid]));
  }
...
void search_engine_cpu::search_worker(const std::string &lyric_to_find,
                                      size_t thread_id,
  ...
  std::copy_if(m_db.begin() + start, m_db.begin() + end,
               std::back_inserter(results), search_pred);
}
