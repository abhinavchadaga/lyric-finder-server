...
static void abort(const int status, const std::string& reason) {
...
const char* LevelHelper::convertToString(const Level level) {
...
void LevelHelper::forEachLevel(base::type::EnumType* startIndex, const std::function<bool(void)>& fn) {
  const base::type::EnumType lIndexMax = LevelHelper::kMaxValid;
  do {
    if (fn()) {
      break;
    }
...
const char* ConfigurationTypeHelper::convertToString(const ConfigurationType configurationType) {
  // Do not use switch over strongly typed enums because Intel C++ compilers dont support them yet.
...
void ConfigurationTypeHelper::forEachConfigType(base::type::EnumType* startIndex, const std::function<bool(void)>& fn) {
  const base::type::EnumType cIndexMax = ConfigurationTypeHelper::kMaxValid;
  do {
    if (fn()) {
      break;
    }
...
Configuration::Configuration(const Level level, const ConfigurationType configurationType, const std::string& value) :
  m_level(level),
...
Configuration::Predicate::Predicate(const Level level, const ConfigurationType configurationType) :
  m_level(level),
  m_configurationType(configurationType) {
}
...
Configurations::Configurations(const std::string& configurationFile, const bool useDefaultsForRemaining,
                               Configurations* base) :
...
bool Configurations::parseFromFile(const std::string& configurationFile, Configurations* base) {
  ...
  const bool success = Parser::parseFromFile(configurationFile, this, base);
  m_isFromFile = success;
  return success;
}

bool Configurations::parseFromText(const std::string& configurationsString, Configurations* base) {
  const bool success = Parser::parseFromText(configurationsString, this, base);
  if (success) {
    m_isFromFile = false;
  }
  return success;
...
bool Configurations::hasConfiguration(const ConfigurationType configurationType) {
...
void Configurations::set(const Level level, const ConfigurationType configurationType, const std::string& value) {
  base::threading::ScopedLock scopedLock(lock());
...
void Configurations::Parser::ignoreComments(std::string* line) {
  ...
  const std::size_t quotesStart = line->find("\"");
...
void Configurations::unsafeSetIfNotExist(const Level level, const ConfigurationType configurationType, const std::string& value) {
  const Configuration* conf = RegistryWithPred<Configuration, Configuration::Predicate>::get(level, configurationType);
  if (conf == nullptr) {
    unsafeSet(level, configurationType, value);
  }
}

void Configurations::unsafeSet(const Level level, const ConfigurationType configurationType, const std::string& value) {
...
void Configurations::setGlobally(const ConfigurationType configurationType, const std::string& value,
                                 const bool includeGlobalLevel) {
...
void Configurations::unsafeSetGlobally(const ConfigurationType configurationType, const std::string& value,
                                       const bool includeGlobalLevel) {
  if (includeGlobalLevel) {
...
void LogBuilder::convertToColoredOutput(base::type::string_t* logLine, const Level level) {
...
void Logger::flush(const Level level, base::type::fstream_t* fs) {
  if (fs == nullptr && m_typedConfigurations->toFile(level)) {
    fs = m_typedConfigurations->fileStream(level);
  }
  if (fs != nullptr) {
    fs->flush();
    const std::unordered_map<Level, unsigned int>::iterator iter = m_unflushedCount.find(level);
...
std::string File::extractPathFromFilename(const std::string& fullPath, const char* separator) {
  ...
  const std::size_t lastSlashAt = fullPath.find_last_of(separator);
  if (lastSlashAt == 0) {
...
void File::buildStrippedFilename(const char* filename, char buff[], const std::size_t limit) {
  const std::size_t sizeOfFilename = strlen(filename);
...
void File::buildBaseFilename(const std::string& fullPath, char buff[], const std::size_t limit, const char* separator) {
...
std::string& Str::replaceAll(std::string& str, const char replaceWhat, const char replaceWith) {
...
bool Str::contains(const char* str, const char c) {
  for (; *str; ++str) {
    if (*str == c)
      return true;
  }
...
char* Str::convertAndAddToBuff(std::size_t n, int len, char* buf, const char* bufLim, const bool zeroPadded) {
  char localBuff[10] = "";
...
char* Str::clearBuff(char buff[], const std::size_t lim) {
  STRCPY(buff, "", lim);
...
bool OS::termSupportsColor(void) {
  const std::string term = getEnvironmentVariable("TERM", "");
...
base::type::string_t DateTime::formatTime(unsigned long long time, base::TimestampUnit timestampUnit) {
  const base::type::EnumType start = static_cast<base::type::EnumType>(timestampUnit);
...
unsigned long long DateTime::getTimeDifference(const struct timeval& endTime, const struct timeval& startTime,
                                               const base::TimestampUnit timestampUnit) {
...
char* DateTime::parseFormat(char* buf, const std::size_t bufSz, const char* format, const struct tm* tInfo,
                            const std::size_t msec, const base::SubsecondPrecision* ssPrec) {
...
void CommandLineArgs::setArgs(const int argc, char** argv) {
  m_params.clear();
...
const char* CommandLineArgs::getParamValue(const char* paramKey) const {
  const std::unordered_map<std::string, std::string>::const_iterator iter = m_paramsWithValue.find(std::string(paramKey));
...
LogFormat::LogFormat(const Level level, const base::type::string_t& format)
...
bool TypedConfigurations::enabled(const Level level) {
  return getConfigByVal<bool>(level, &m_enabledMap, "enabled");
}

bool TypedConfigurations::toFile(const Level level) {
  return getConfigByVal<bool>(level, &m_toFileMap, "toFile");
}

const std::string& TypedConfigurations::filename(const Level level) {
  return getConfigByRef<std::string>(level, &m_filenameMap, "filename");
}

bool TypedConfigurations::toStandardOutput(const Level level) {
  return getConfigByVal<bool>(level, &m_toStandardOutputMap, "toStandardOutput");
}

const base::LogFormat& TypedConfigurations::logFormat(const Level level) {
  return getConfigByRef<base::LogFormat>(level, &m_logFormatMap, "logFormat");
}

const base::SubsecondPrecision& TypedConfigurations::subsecondPrecision(const Level level) {
  return getConfigByRef<base::SubsecondPrecision>(level, &m_subsecondPrecisionMap, "subsecondPrecision");
}

const base::MillisecondsWidth& TypedConfigurations::millisecondsWidth(const Level level) {
  return getConfigByRef<base::MillisecondsWidth>(level, &m_subsecondPrecisionMap, "millisecondsWidth");
}

bool TypedConfigurations::performanceTracking(const Level level) {
  return getConfigByVal<bool>(level, &m_performanceTrackingMap, "performanceTracking");
}

base::type::fstream_t* TypedConfigurations::fileStream(const Level level) {
  return getConfigByRef<base::FileStreamPtr>(level, &m_fileStreamMap, "fileStream").get();
}

std::size_t TypedConfigurations::maxLogFileSize(const Level level) {
  return getConfigByVal<std::size_t>(level, &m_maxLogFileSizeMap, "maxLogFileSize");
}

std::size_t TypedConfigurations::logFlushThreshold(const Level level) {
...
std::string TypedConfigurations::resolveFilename(const std::string& filename) {
  ...
  std::size_t dateIndex = std::string::npos;
  const std::string dateTimeFormatSpecifierStr = std::string(base::consts::kDateTimeFormatSpecifierForFilename);
  if ((dateIndex = resultingFilename.find(dateTimeFormatSpecifierStr.c_str())) != std::string::npos) {
    ...
    if (dateIndex != std::string::npos) {
      ...
      const base::SubsecondPrecision ssPrec(3);
...
void TypedConfigurations::insertFile(const Level level, const std::string& fullFilename) {
  ...
  const std::string filePath = base::utils::File::extractPathFromFilename(resolvedFilename, base::consts::kFilePathSeparator);
  if (filePath.size() < resolvedFilename.size()) {
    base::utils::File::createPath(filePath);
  }
  auto create = [&](Level level) {
    const base::LogStreamsReferenceMap::iterator filestreamIter = m_logStreamsReference->find(resolvedFilename);
...
bool TypedConfigurations::unsafeValidateFileRolling(const Level level, const PreRollOutCallback& preRollOutCallback) {
  base::type::fstream_t* fs = unsafeGetConfigByRef(level, &m_fileStreamMap, "fileStream").get();
  if (fs == nullptr) {
    return true;
  }
  const std::size_t maxLogFileSize = unsafeGetConfigByVal(level, &m_maxLogFileSizeMap, "maxLogFileSize");
  const std::size_t currFileSize = base::utils::File::getSizeOfFile(fs);
  if (maxLogFileSize != 0 && currFileSize >= maxLogFileSize) {
    const std::string fname = unsafeGetConfigByRef(level, &m_filenameMap, "filename");
...
bool RegisteredHitCounters::validateEveryN(const char* filename, const base::type::LineNumber lineNumber, const std::size_t n) {
  base::threading::ScopedLock scopedLock(lock());
  ...
  const bool result = (n >= 1 && counter->hitCounts() != 0 && counter->hitCounts() % n == 0);
  return result;
}
...
bool RegisteredHitCounters::validateAfterN(const char* filename, const base::type::LineNumber lineNumber, const std::size_t n) {
  base::threading::ScopedLock scopedLock(lock());
  base::HitCounter* counter = get(filename, lineNumber);
  if (counter == nullptr) {
    registerNew(counter = new base::HitCounter(filename, lineNumber));
...
/// @return True if validation resulted in triggering hit. Meaning logs should be written everytime true is returned
bool RegisteredHitCounters::validateNTimes(const char* filename, const base::type::LineNumber lineNumber, const std::size_t n) {
  base::threading::ScopedLock scopedLock(lock());
  base::HitCounter* counter = get(filename, lineNumber);
  if (counter == nullptr) {
    registerNew(counter = new base::HitCounter(filename, lineNumber));
...
Logger* RegisteredLoggers::get(const std::string& id, const bool forceCreation) {
  base::threading::ScopedLock scopedLock(lock());
  Logger* logger_ = base::utils::Registry<Logger, std::string>::get(id);
  if (logger_ == nullptr && forceCreation) {
    const bool validId = Logger::isValidId(id);
    if (!validId) {
...
VRegistry::VRegistry(const base::type::VerboseLevel level, base::type::EnumType* pFlags) : m_level(level), m_pFlags(pFlags) {
}

/// @brief Sets verbose level. Accepted range is 0-9
void VRegistry::setLevel(const base::type::VerboseLevel level) {
  base::threading::ScopedLock scopedLock(lock());
  if (level > 9)
...
void VRegistry::setModules(const char* modules) {
  ...
  auto addSuffix = [](std::stringstream& ss, const char* sfx, const char* prev) {
    if (prev != nullptr && base::utils::Str::endsWith(ss.str(), std::string(prev))) {
      const std::string chr(ss.str().substr(0, ss.str().size() - strlen(prev)));
      ss.str(std::string(""));
      ss << chr;
    }
    if (base::utils::Str::endsWith(ss.str(), std::string(sfx))) {
      const std::string chr(ss.str().substr(0, ss.str().size() - strlen(sfx)));
      ss.str(std::string(""));
      ss << chr;
    }
    ss << sfx;
...
bool VRegistry::allowed(const base::type::VerboseLevel vlevel, const char* file) {
  base::threading::ScopedLock scopedLock(lock());
...
bool Storage::uninstallCustomFormatSpecifier(const char* formatSpecifier) {
  base::threading::ScopedLock scopedLock(customFormatSpecifiersLock());
  const std::vector<CustomFormatSpecifier>::iterator it = std::find(m_customFormatSpecifiers.begin(),
...
void Storage::setApplicationArguments(const int argc, char** argv) {
...
base::threading::Mutex& LogDispatchCallback::fileHandle(const LogDispatchData* data) {
  const auto it = m_fileLocks.find(data->logMessage()->logger()->typedConfigurations()->filename(data->logMessage()->level()));
...
base::type::string_t DefaultLogBuilder::build(const LogMessage* logMessage, const bool appendNewLine) const {
  ...
  const el::base::threading::ScopedLock lock_(ELPP->customFormatSpecifiersLock());
  ELPP_UNUSED(lock_);
...
Writer& Writer::construct(Logger* logger, const bool needLock) {
  m_logger = logger;
...
Writer& Writer::construct(const int count, const char* loggerIds, ...) {
...
void Writer::initializeLogger(const std::string& loggerId, const bool lookup, const bool needLock) {
  if (lookup) {
...
Logger* Loggers::getLogger(const std::string& identity, const bool registerIfNotAvailable) {
...
Logger* Loggers::reconfigureLogger(const std::string& identity, const ConfigurationType configurationType,
...
void Loggers::reconfigureAllLoggers(const Level level, const ConfigurationType configurationType,
                                    const std::string& value) {
  for (base::RegisteredLoggers::iterator it = ELPP->registeredLoggers()->begin();
       it != ELPP->registeredLoggers()->end(); ++it) {
...
void Loggers::setDefaultConfigurations(const Configurations& configurations, const bool reconfigureExistingLoggers) {
...
void Loggers::setVerboseLevel(const base::type::VerboseLevel level) {
